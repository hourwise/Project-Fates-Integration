...
9. Define the next Integration slice as one pinned Moirae/Horae → Ananke harmless or read-only canonical action request and traceable decision round trip. Preserve origin, schema identity, both principals and correlation; prove no bypass path was used. Do not claim or require an external side effect in that slice.

## Blocking assessment and safest order

No finding blocks continued documentation, local refactoring that preserves behaviour, research, or the narrow action-decision round trip above. Each capability is blocked only by its corresponding gate: consequential effects require exact approval/live-state binding and a non-bypassable chokepoint; retryable effects require persistent idempotency; modern/remote/headless MCP requires the protocol and authorization gates; imported workflows require import analysis; durable derived memory requires dependency/ACL invalidation. The next recommended slice requires approval-mutation binding, a bypass inventory, origin/schema receipts, readiness proof, live-state handling and end-to-end correlation, but it does not require workflow recovery, remote OAuth, provider fallback, derived understanding or full content preflight.

Safest order: decide only the gates required for the narrow action-decision round trip → prove that pinned cross-process path → add persistent idempotency before retryable effects → add Moirae isolation before local effects → add MCP authorization/version gates before remote or modern MCP → add durable workflow, information-flow, preflight, derived-memory and fallback controls immediately before enabling their respective capability classes → add optional dashboards and adapters last.
